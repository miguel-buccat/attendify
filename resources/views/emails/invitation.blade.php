...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>You're invited to join {{ $institutionName }}</title>
    </head>
                    ...
                    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:620px;background-color:#ffffff;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;">
                        <tr>
                            <td style="padding:24px 24px 12px 24px;background-color:#f9fafb;border-bottom:1px solid #e5e7eb;">
                                ...
                                <p style="margin:6px 0 0 0;font-size:14px;line-height:1.4;color:#4b5563;">Account Invitation</p>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td style="padding:24px;">
                                ...
                                <p style="margin:0 0 18px 0;font-size:15px;line-height:1.6;">
                                    You have been invited to join <strong>{{ $institutionName }}</strong> as a
                                    <strong>{{ $invitation->role->value }}</strong>.
                                    Click the button below to create your account and get started.
                                </p>

                                <p style="margin:0 0 20px 0;">
                                    <a href="{{ $acceptUrl }}" style="display:inline-block;background-color:#2563eb;color:#ffffff;text-decoration:none;font-weight:600;padding:12px 18px;border-radius:8px;">Accept Invitation</a>
                                </p>

                                <p style="margin:0 0 10px 0;font-size:14px;line-height:1.6;color:#4b5563;">
                                    This invitation will expire on <strong>{{ $invitation->expires_at->format('F j, Y') }}</strong>.
                                ...
                                <p style="margin:0;font-size:13px;line-height:1.6;color:#2563eb;word-break:break-word;">{{ $acceptUrl }}</p>
                            </td>
                        </tr>
